...
public class CustomerEditViewModel : ViewModelBase
{
    ...
    public CustomerEditViewModel(Customer existing)
    {
        ...
        if (existing.LicenseExpiry.HasValue)
            LicenseExpiry = new DateTimeOffset(existing.LicenseExpiry.Value, TimeSpan.Zero);
        if (existing.DateOfBirth.HasValue)
            DateOfBirth = new DateTimeOffset(existing.DateOfBirth.Value, TimeSpan.Zero);
        ...
        this.WhenAnyValue(x => x.Email).Subscribe(_ => ValidateEmailFormat());
        this.WhenAnyValue(x => x.Pesel).Subscribe(_ => ValidatePeselFormat());
    }
    ...
    private bool Validate()
    {
        ...
        return FirstNameError == null && LastNameError == null && PhoneError == null &&
               IdDocumentError == null && LicenseNumberError == null &&
               EmailError == null && PeselError == null;
    }
